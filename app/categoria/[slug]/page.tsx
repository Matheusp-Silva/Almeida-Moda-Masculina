...
import { MessageCircle, ArrowLeft, ShoppingBag, Menu } from "lucide-react";
...
export default function CategoryPage() {
  ...
  const { slug } = params; // Aqui pegamos "roupas-social", "calcados-tenis", etc.
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      // Query filtrando pela categoria específica
  ...
  const getWhatsAppLink = (productName: string, price: number) => {
    const message = `Olá! Vi o *${productName}* (R$ ${price.toFixed(2)}) na categoria *${slug}* e tenho interesse.`;
    return `https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(message)}`;
  };

  // Formata o título da página baseado no slug (ex: roupas-camisetas -> Camisetas)
    ...
    <div className="bg-almeida-branco text-almeida-preto min-h-screen font-sans selection:bg-almeida-vermelho selection:text-white">
      ...
      <nav className="flex items-center justify-between px-6 md:px-12 h-20 bg-almeida-branco/95 backdrop-blur-sm sticky top-0 z-50 border-b border-gray-100">
        ...
        <ul className="hidden md:flex h-full items-center gap-8 font-bold text-[13px] uppercase tracking-tight">
             ...
             <li className="text-almeida-vermelho cursor-default border-b-2 border-almeida-vermelho">
                {formatTitle(slug || "")}
             </li>
        </ul>
      ...
      <header className="bg-[#f5f5f5] py-16 px-6 md:px-12 mb-12">
        <div className="max-w-7xl mx-auto">
            <Link href="/" className="inline-flex items-center gap-2 text-xs font-black uppercase tracking-widest text-gray-400 hover:text-almeida-vermelho mb-4 transition-colors">
                <ArrowLeft size={14} /> Voltar para Home
            </Link>
            <h1 className="text-5xl md:text-7xl font-[900] uppercase italic tracking-tighter text-almeida-preto">
                {formatTitle(slug || "")}
            </h1>
      ...
      <section className="px-6 md:px-12 pb-24 max-w-7xl mx-auto">
        {products.length > 0 ? (
          ...
          <div className="text-center py-20 bg-gray-50 rounded-lg border border-gray-100">
            <h3 className="text-xl font-black uppercase italic text-gray-300 mb-2">Sem estoque no momento</h3>
            <p className="text-gray-400 text-sm">Nenhum produto encontrado nesta categoria.</p>
            <Link href="/" className="mt-6 inline-block text-almeida-vermelho font-bold uppercase text-xs tracking-widest border-b border-almeida-vermelho pb-1">
                Ver outros produtos
            </Link>
          </div>
        )}
